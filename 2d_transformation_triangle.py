#Simple program of triangle transformation
...
if __name__ == "__main__":
    ...
    M1 = np.array([[round(math.cos(math.radians(theta)),2), -round(math.sin(math.radians(theta)),2), 0],
                   [round(math.sin(math.radians(theta)),2), round(math.cos(math.radians(theta)),2), 0],
                   [0, 0, 1]])

    M2 = np.array([[x1, x2, x3],
                   [y1, y2, y3],
                   [1, 1, 1]])

    Mul = np.matmul(M1, M2)

    Mul = np.delete(Mul, 2, 0)
    Mul = Mul.T #to transpose
